...
class DecisionTree():
    ...
    def predict(self, X):
        ...
        for x in X:
         ...
         #make sure the tree wont be messed up by classifying
    ...
    def entropy(self, labels):
        ...
        for target in set(labels):
            count = list(labels).count(target)
            target_dict[target] = count
...
class TreeNode:
    ...
    def classify(self, x, attributes, default_value):
       ...
       '''
       #once we reach to a leaf, we are done
       if self.is_leaf():
           return self.value
       #else, we need to check the node and its children
       else:
         node = self.attribute
         children_dict = self.children
         att_index = attributes.index(node)
         a_val = x[att_index]
         #track which branch we should pass, and get the node from the branch
         if a_val in children_dict:
           children = children_dict[a_val]
         else:
          return default_value
         if children:
              #we are done if we arrive at a leaf
              if children.is_leaf():
