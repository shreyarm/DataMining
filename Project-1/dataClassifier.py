...
def readCommand(argv):
    ...
    parser.add_option('-c', '--classifier', help=default('The type of classifier'),
                      choices=['predictOne', 'zeroR', 'firstFeatureClf', 'nb', 'naiveBayes', #'perceptron',
                               'minicontest'], default='predictOne')
    ...
    if options.smoothing < 0:
        print "Please provide zero or a positive number for smoothing (you provided: %f)" % options.smoothing
        print USAGE_STRING
        sys.exit(2)
    ...
    if options.classifier == 'predictOne':
        classifier = predictOne.predictOne(legalLabels)
    elif options.classifier == 'firstFeatureClf':
        classifier = firstFeatureClassifier.firstFeatureClassifier(legalLabels)
    elif options.classifier == "zeroR":
        classifier = zeroR.zeroR(legalLabels)
